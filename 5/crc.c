...
char g[]="10011"; //generator polynomial
int a,e,c;
...
void crc()
{
    ...
    do{
        ...
        cs[c]=t[e++];
	printf("\nRemainder %d:",e-N);
	puts(cs); // as we pushed the element at the 0th index out , we take the next element from the padded data to our xor operand.
    }while(e<=a+N-1);
 // we continue this process till all the extra elements from the padded data are added to our xor operand and the division process is complete.
}
